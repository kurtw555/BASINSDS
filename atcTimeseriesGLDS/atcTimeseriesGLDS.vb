...
''' <summary>
''' Reads GrADS Data Server ASCII or GLDAS timeseries.cgi files containing time-series values
''' </summary>
''' <remarks>
...
''' </remarks>
Public Class atcTimeseriesGLDS
    ...
    Public Overrides ReadOnly Property Description() As String
        Get
            Return "NASA GLDAS File"
        End Get
    End Property

    Public Overrides ReadOnly Property Name() As String
        Get
            Return "Timeseries::NASA GLDAS"
        End Get
    End Property
    ...
    Private Function ReadASC2(ByVal lInputReader As BinaryReader) As Boolean
            ...
            If lCurLine.Length = 2 Then
                ...
                Select Case lAttName
                    ...
                    Case "lon"
                        ...
                        'GPF added 11/12/20, GLDAS dose not have elevation
        ...
        Do
            Try
                lCurLineString = NextLine(lInputReader)
                'GPF 11/11/2020; convert units (moved from GLDAS class)
                If Date.TryParse(SafeSubstring(lCurLineString, 0, 19).Trim, lDate) AndAlso
                    ...
                    irec += 1
                    If (lValue < -990) Then
                        lValue = 9999 'change missing to 9999
                    Else
                        Select Case lCons
                            Case "Wind_f_inst"
                                lValue = lValue * 2.237 'mph  
                            Case "Rainf_tavg"
                                lValue = lValue * 3600 / 25.4 'in
                            Case "Tair_f_inst"
                                lValue = (lValue * 9 / 5) - 459.67 'F
                            Case "Psurf_f_inst"
                                lValue = lValue * 0.00750062 'mmhg
                            Case "LWdown_f_tavg"
                                lValue = lValue * 0.085985 'langley
                            Case "SWdown_f_tavg"
                                lValue = lValue * 0.085985 'langley
                        End Select
                    End If
                    dictTS.Add(lDate, lValue)
                End If
    ...
    Private Function InterpolateHourly(lParam As String, lDate As Date(), lValue As Double()) As SortedDictionary(Of Date, Double)
        ...
        If lParam.ToUpper().Contains("RAINF") Then
        ...
        Else
            For idx As Integer = 1 To lDate.Length - 1
                ...
                'interpolate
                Dim diff = (lValue(idx) - prelValue) / dtdiff
                For ihr As Integer = 1 To dtdiff - 1
                    ldt = prelDate.AddHours(ihr)
                    lval = prelValue + (ihr * diff)
                    dictTS.Add(ldt, lval)
                    'Debug.WriteLine("{0},{1},{2}", ihr.ToString(), ldt.ToString(), lval.ToString("0.000000"))
                Next
                dictTS.Add(lDate.ElementAt(idx), lValue.ElementAt(idx))
                'Debug.WriteLine("{0},{1}", lDate.ElementAt(idx).ToString(), lValue.ElementAt(idx).ToString("0.000000"))
                prelDate = lDate(idx)
                prelValue = lValue(idx)
            Next
        End If
        Return dictTS
    End Function
